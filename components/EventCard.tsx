...
const EventCard: React.FC<EventCardProps> = ({ event }) => {
  ...
  const hasParticipated = requests.some(
  ...
  return (
    <>
      <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl overflow-hidden flex flex-col border border-slate-100 dark:border-slate-800">
        ...
        <div className="p-5 sm:p-6 flex-grow flex flex-col justify-between">
          ...
          >
            {hasParticipated ? 'Inscrição Pendente' : 'Quero Participar'}
          </button>
        </div>
      </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-t-[2rem] sm:rounded-[2.5rem] shadow-2xl p-6 sm:p-10 w-full max-w-xl border-t sm:border border-slate-200 dark:border-slate-800 max-h-[90vh] overflow-y-auto">
            ...
            <div className="text-sm text-slate-600 dark:text-slate-400 space-y-4 font-bold">
              ...
              <ul className="space-y-4 text-xs uppercase tracking-wider">
                  ...
                  <li className="flex gap-3 items-start">
                    <span className="text-emerald-500 font-black">✓</span>
                    <span className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 block w-full">
            ...
            {message && (
                <div className={`mt-6 p-4 rounded-xl font-black text-xs text-center border ${message.type === 'success' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-red-50 text-red-700 border-red-100'}`}>
                    {message.text}
                </div>
            )}
