...
const UserRequestsPage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-4 sm:py-10 space-y-6 sm:space-y-8">
      <div className="flex items-center gap-3 sm:gap-4 px-2">
        <button 
          onClick={() => navigate('/')} 
          className="p-3 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border-2 border-slate-100 dark:border-slate-700 active:scale-95"
        >
              ...
              <div key={req.id} className="p-5 sm:p-8 flex flex-col gap-5 hover:bg-slate-50/30 transition-colors">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black tracking-widest uppercase border ${
                        ...
                        {req.status === 'completed' ? 'CONCLUÍDO' : 'EM ANDAMENTO'}
                      </span>
                  ...
                  <div className="flex items-center sm:flex-col sm:items-end gap-2">
                     <div className={`flex-1 sm:flex-none text-center px-4 py-2.5 rounded-xl border-2 font-black text-[10px] uppercase tracking-wider ${
                ...
                {req.status === 'waiting_receipt' && (
                  <div className="bg-blue-50 dark:bg-blue-900/10 p-5 sm:p-6 rounded-2xl border-2 border-blue-100 dark:border-blue-900/50 space-y-4 shadow-inner">
                    <p className="text-blue-800 dark:text-blue-300 font-bold text-xs sm:text-sm leading-relaxed">
                      O Admin aprovou! Você deve ter recebido o Pix total. Agora devolva <strong className="text-blue-950 dark:text-blue-100">R$ {(parseFloat(req.eventValue) * 0.75).toFixed(2)}</strong> (75%) para o admin manter seu acesso e poder se inscrever novamente.
                    </p>
                    <div className="flex flex-col gap-3">
                       <div className="flex-1">
                         <label className="text-[9px] font-black text-blue-400 uppercase tracking-widest block mb-1 ml-1">Chave PIX Admin</label>
                         <div className="flex items-center bg-white dark:bg-slate-800 px-4 py-4 rounded-xl border-2 border-blue-200 dark:border-blue-800 justify-between">
                ...
                {req.status === 'paid' && (
                  <div className="p-6 bg-purple-50 dark:bg-purple-900/10 rounded-2xl border-2 border-purple-100 dark:border-purple-800 flex flex-col items-center gap-3 text-center">
                    <div className="w-12 h-12 bg-purple-100 text-purple-600 rounded-2xl flex items-center justify-center animate-bounce">
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    </div>
                    <p className="text-[11px] font-black text-purple-700 dark:text-purple-300 uppercase tracking-widest">Devolução em validação...</p>
                    <p className="text-[9px] font-bold text-slate-400">Após validado, o evento será liberado para nova inscrição.</p>
                  </div>
                )}
